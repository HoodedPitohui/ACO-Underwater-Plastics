...
%% Set File Names
trashDatasheet = '../data/trashData.xlsx';
sheetName = 'sheet0';
trialName = 'trial1';
regionName = 'Region1';
...
for d = 1: droneNo
    ...
    while t <= environment.maxIter && drones.bestSolutionsFound(d) ~= 1
        ...
        for k = 1: 1: drones.antNo
            if (t == 1 && k == 1 ||(drones.colony(d).ant(k).distFitness > 0 && ...
                    drones.colony(d).ant(k).distFitness < drones.besttrashDist(1, d)...
                    && drones.colony(d).ant(k).trashFitness < ...
                    5 + drones.besttrashFitness(1, d)))
